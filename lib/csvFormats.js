...
export function detectFormat(headers) {
  ...
  if (h.includes('pair') && h.includes('risk_pts') && h.includes('entry') && h.includes('target')) {
    return 'r_multiple_model';
  }
...
export function convertOrderLevelRows(rows) {
  ...
  for (const pid in byPosition) {
    ...
    trades.push({ symbol, direction, entry_price, exit_price, size, entry_time, exit_time, pnl, ticket: pid, notes: '' });
  }
...
function parseIsoWithOffset(str) {
  if (!str) return null;
  // "2024-06-06 18:00:00-04:00" -> "2024-06-06T18:00:00-04:00" (already has a
  // real UTC offset, so this parses correctly regardless of the reader's
  // own timezone)
...
// Backtested/model trade logs (e.g. futures strategy exports) that record
// R-multiples and price levels but NO dollar P&L or contract size at all —
// dollarPerR converts each trade's R-multiple into a dollar pnl. Exit price
// is back-calculated from entry + R*risk_pts so the trade is still treated
// as "closed" everywhere else in the app (stats filter on exit_price being
// set) — it's a derived, consistent price, not a literal broker fill.
export function convertRMultipleModelRows(rows, dollarPerR) {
  const perR = parseFloat(dollarPerR) || 0;
  return rows
    .filter((r) => r['pair'] && r['entry'] && r['R'] !== undefined && r['R'] !== '')
    .map((r) => {
      const direction = (r['side'] || '').trim().toLowerCase() === 'bear' ? 'short' : 'long';
      const entry = parseFloat(r['entry']);
      const riskPts = parseFloat(r['risk_pts']);
      const rMult = parseFloat(r['R']);
      const priceMove = rMult * riskPts;
      const exitPrice = direction === 'long' ? entry + priceMove : entry - priceMove;
      const stopLoss = direction === 'long' ? entry - riskPts : entry + riskPts;

      return {
        symbol: (r['pair'] || '').trim(),
        direction,
        entry_price: entry,
        exit_price: isNaN(exitPrice) ? '' : exitPrice,
        size: '',
        entry_time: parseIsoWithOffset(r['dt_et']),
        exit_time: parseIsoWithOffset(r['exit_et']),
        pnl: perR ? rMult * perR : 0,
        stop_loss: isNaN(stopLoss) ? '' : String(stopLoss),
        take_profit: r['target'] || '',
        risk_amount: perR || '',
        ticket: '',
        notes: perR
          ? `Model backtest trade — ${rMult}R at $${perR}/R. Exit price is back-calculated from R, not a literal fill.`
          : `Model backtest trade — ${rMult}R (no $/R set, pnl is $0).`,
      };
    })
    .sort((a, b) => (a.entry_time || '').localeCompare(b.entry_time || ''));
}
