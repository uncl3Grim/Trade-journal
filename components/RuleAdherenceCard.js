...
export default function RuleAdherenceCard({ trades }) {
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-2xl shadow-sm p-5 mb-4">
      <h3 className="font-semibold text-gray-900 mb-1 text-sm">Rule Adherence</h3>
      <p className="text-xs text-gray-400 mb-3">
        How much of your result came from following your plan vs. deviating from it.
      </p>
      {rows.length === 0 ? (
        <p className="text-xs text-gray-400">Tag trades with rule adherence to see this breakdown.</p>
      ) : (
        rows.map((r) => {
          ...
          return (
            <div key={r.label} className="flex items-center gap-3 py-1.5">
              <div className="w-32 text-xs text-gray-500 flex-shrink-0">
                {LABELS[r.label] || r.label} <span className="text-gray-300">{r.count}</span>
              </div>
