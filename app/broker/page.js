...
function BalanceEditor({ connection, onSaved }) {
  ...
  return (
    <div className="flex items-center gap-2 mt-2">
      ...
      />
      <button onClick={save} disabled={saving} className="text-xs text-indigo-600 hover:text-indigo-500 disabled:opacity-50">
        {saving ? 'Saving...' : 'Save'}
      </button>
    </div>
  );
...
function NameEditor({ connection, onSaved }) {
  ...
  return (
    <div className="flex items-center gap-2">
      <input autoFocus value={value} onChange={(e) => setValue(e.target.value)} className="bg-white border border-gray-300 rounded-lg px-2 py-1 text-sm text-gray-900" />
      <button onClick={save} disabled={saving} className="text-xs text-indigo-600 hover:text-indigo-500 disabled:opacity-50">
        {saving ? '...' : 'Save'}
      </button>
      <button onClick={() => { setEditing(false); setValue(connection.broker_server); }} className="text-xs text-gray-400 hover:text-gray-600">
        Cancel
      </button>
    </div>
  );
...
export default function BrokerPage() {
  ...
  async function handleDelete(connectionId, label) {
    ...
    setDeletingId(null);
    if (error) setSyncMessage(`Error deleting: ${error.message}`);
    ...
    <AppShell>
      <div className="max-w-3xl mx-auto px-4 py-6">
        ...
        <div className="space-y-3">
          {connections.length === 0 && <p className="text-sm text-gray-400">No accounts connected yet.</p>}
          {connections.map((c) => (
